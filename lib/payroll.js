...
function splitShift(shift, paymentType, isPH) {
...
function buildBuckets(shifts, paymentType, rates, isPH) {
  ...
  for (const s of shifts || []) {
    ...
    const splits = splitShift(s, paymentType, ph);
...
// Auto-compute meal + fares allowances for TFN. Returns 0/0 for other sections.
function computeAutoAllowances(shifts, paymentType, rates) {
  if (paymentType !== 'tfn') return { travel: 0, meal: 0 };
  const dates = new Set();
  let longCount = 0;
  for (const s of shifts || []) {
    if (s.date) dates.add(s.date);
    if (toNum(s.hours) >= 10) longCount++;
  }
  return {
    travel: round2(dates.size * toNum(rates.fares)),  // travel_allowance ← fares per day worked
    meal:   round2(longCount   * toNum(rates.meal)),  // meal_allowance ← per shift >= 10h
  };
}
...
function recomputeLine(line, { isPH } = {}) {
  ...
  const grand = round2(total_wages + totalAllowance);
