 ...
 */
// Normalise legacy response shape ({ checked, na }) to new state ('yes'|'no'|'na'|'')
  ...
  const groups = SCORE_GROUPS.map(g => {
    ...
    for (const secKey of g.sectionKeys) {
      ...
      section.items.forEach((_, idx) => {
        ...
        const state = normaliseState(responses[key]);
...
module.exports = { AUDIT_SECTIONS, SCORE_GROUPS, itemKey, computeScore, normaliseState };
