...
function autoLogDiary(db, { jobId, category, complianceItemId, summary, userId }) {
  ...
  try {
    ...
    // Keep job's last_update_date current so "missing weekly update" stays accurate
    try { db.prepare('UPDATE jobs SET last_update_date = ? WHERE id = ?').run(today, jobId); } catch (e) { /* ignore */ }
  } catch (e) {
 ...
 */
function logStatusChange(db, { jobId, entityType, entityLabel, oldStatus, newStatus, userId, userName, category }) {
  ...
  // Derive category from entityType if not explicitly provided
  const diaryCategory = category || (entityType === 'task' ? 'Task Status Change' : entityType === 'compliance' ? 'Compliance Update' : 'Status Update');

  // 1. Auto-log to site diary
  autoLogDiary(db, { jobId, category: diaryCategory, summary, userId });
