...
- **Brand = T&S indigo**: brand-700 `#13008E` (the logo blue), brand-600 `#2517A6`, brand-500 `#3D31C0` (tokens at the top of custom.css; full 50–950 ramp in the inline Tailwind config, views/layout.ejs). Signal tones: amber `#FBBF24` warn, red `#EF4444` danger, blue `#60A5FA` info; success green is semantic only, never brand. (The old blue `#2B7FFF` era is gone — don't reintroduce it; primary controls use `var(--lg-brand-fill)`.)
...
- **Theming is dark-first**: `body.dark-glass` is always on; light mode is the `:root[data-theme="light"]` override layer. 15 user-selectable themes (`window.ATOMIS_THEMES` in layout.ejs; default `ts` is LIGHT; skins re-tint the ground via themes.css). **Liquid Glass** (Sep 2026, public/css/liquid-glass.css, loaded last): `bg-white … rounded-xl/2xl` cards automatically become the glass material in BOTH themes (blur + specular rim + 16px radius); bare `bg-white` is tint-only; nested `.bg-white .bg-white` is a flat well. New markup using the canonical idiom gets it for free.
- **Key CSS layers**: custom.css (tokens + dark-glass remaps) → themes.css (skins) → admin-fluid.css (view-transition motion) → admin-polish.css (§9 `.stat-card`, §11 page title/eyebrow, §12 `.chevron-rail`/`.chevron-rail-left` hazard striping, §13 `.lane-divider`, §17 dashboard devices: `.panel-header`, `.attn-row`, `.hero-board`) → **liquid-glass.css** (the material: `--lg-*` tokens, `.lg` / `.lg-strong` / `.lg-chrome` / `.lg-flat` / `.lg-seg` / `.lg-capsule` / `.lg-none`, the `#lg-ambient` field, shell, controls, bookings/dashboard device sections, motion, fallbacks — §F must stay last). Reuse these before inventing new ones.
- **Liquid Glass rules**: never write `backdrop-filter` by hand — use the `.lg*` classes (or `var(--lg-bg…)` tokens) so the phone budget, `prefers-reduced-transparency`, `@supports` and print fallbacks keep working. Blur belongs on floating chrome, overlays and top-level cards only; nested surfaces use `.lg-flat` / `var(--lg-well)`. Never put `.lg*` on an element that already uses `::before`. Never put transform / filter / backdrop-filter on `body`, `main`, `.bk2-page`, `#rec-root` or any ancestor of a `position:fixed` overlay (containing-block trap). Primary controls = `var(--lg-brand-fill)` + white ink. `.bk2-slot*` colour rules are spec-pinned — don't restyle them. Segmented controls: `.lg-seg` + `data-lg-segment` (thumb from public/js/lg-motion.js; `="underline"` variant for tab strips). Cache bumps live in three places: the `?v=` strings in layout.ejs, `CACHE_NAME` in public/admin-sw.js, and the `/admin-sw.js?v=` registration.
...
- **Sidebar nav**: registry-driven from `lib/sidebarNav.js` (9 sections; section headers ARE the dept-hub links and keep the `sidebar-link` class so the drag-drop customiser's pathname-keyed layouts survive). Never register two hrefs differing only by query string — the customiser dedupes by pathname. Messages lives in the header (`#header-chat`, badge class `chat-unread-badge` live-updates via public/js/chat.js). Aug 2026 redesign (per the Control Room design review): colours ride `--sb-*` CSS vars (custom.css defines dark+light; ts/ts-dark skins re-point them); ACTIVE class is the bare literal `bg-white/15` token (customiser auto-open, e2e and CSS all key on it — IDLE must NEVER contain that substring, that was the rail-on-every-row bug) with the visual = var fill + text, no ::before rail; `currentPage` set via `locals.currentPage=` inside views never reaches the layout (express-ejs-layouts re-renders from original locals) — the sidebar falls back to `res.locals.currentPath` href matching; one badge per link (danger wins, 99+ cap, `sidebar-badge-crit`/`-muted`) and no unbounded totals in the nav; nav filter + collapsed-folder crit rollups + first-load scroll-to-active live in an inline tools script at the bottom of sidebar.ejs (re-runs on the customiser's `sb:rendered` event; filter classes `sb-hidden/sb-nomatch/sb-filtering` never touch stored collapse state).
...
- **Compliance / "Plans & Approvals"** (`/compliance`, `compliance` table, sidebar "Plans & Approvals") — **the module the team actually uses.** Parent plans → sub-plans with refs `TSCA` (council_permit), `TSROL` (rol), `TSTMP` (CTMP/tmp_approval), `TSTGS`, `TSSPA`, etc. Owner, status workflow, fees, dates, revisions live here. Council/ROL/CTMP features belong here. TGS↔ROL links are **many-to-many** via `compliance_tgs_rol_links` (mig 332, package level) and `compliance_doc_rol_links` (mig 356, per TGS sheet = per file) — `compliance.linked_rol_id` is retired (unread, unwritten; left in place because SQLite column drops rewrite the table). **Unlinked = covered by every ROL on the plan** (display rule in `resolveRolCoverage`, routes/compliance.js — no rows written); links only ever narrow, so the ROL-table tab ticks an approved ROL on old plans nobody linked. Sub-plan documents: attach-only via `POST /sub-plans/:id/documents` (no status change); `upload-submit` remains the explicit submission (non-ROL types). Aug 2026 redesign:
  ...
  - **Money = plan-level quote** (`compliance_quote_revisions` + `compliance_quote_lines`, mig 351; mig 352 migrated old sub-plan charges and ZEROED them). The quote FOLLOWS the sub-plans: `syncQuoteLinesFromSubPlans` (edit-loader) seeds one $0 line per sub-plan, deduped via `compliance_quote_lines.sub_plan_id` (mig 353, FK SET NULL so deleting a sub keeps its priced line); each line has dated comments (`compliance_quote_line_comments`) that copy forward into new revisions. `rollupQuoteTotal` denormalises the current revision's total onto the parent's `charge_amount/charge_client` (frozen once `invoiced=1`) — the invoice workflow/register/hub/P&L still read those columns; `finance-pnl.js` reads the parent, not sub sums. The per-sub-plan `/charge` route + UI are gone. Itemised `compliance_fees` show on EVERY sub-plan type (collapsed "Fees" / "Council fees"), and each fee is editable after the fact (`POST /sub-plans/:id/fees/:feeId` — description, amount, optional replacement receipt; blank fields keep their value); totals roll up via `rollupCouncilFee`.
  - **Council extensions are sub-plans** (Sep 2026, mig 361 `compliance.extension_of_id`): `POST /sub-plans/:id/extension-plan` on a council application creates a NEW `council_permit` sub-plan under the same parent (next TSCA seq, description "<permit> — extension", type/owner/job copied, optional new `job_date` + `notes`), linked back through `extension_of_id`; the original gets `extension_required = 1` (recomputed on delete via `recomputeExtensionFlag`). Extensions always hang off the ORIGINAL (no chains). The card shows "EXT of <ref>" / "<n> ext" chips and a banner linking both ways; the register flags "extension" / "extended". ROL extensions are unchanged (dated records that move the end date). Legacy dated council extension records stay visible until deleted.
  ...
  - The service worker must **not** `respondWith(fetch(request))` for API/JSON requests: Chromium races its own network request against the fetch handler, so the handler's `fetch()` became a second server hit for every JSON call (doubled audit rows). API requests fall through to the browser untouched; only static assets (cache-first) and HTML navigations (network + offline fallback) are intercepted.
...
## Pay runs (`/payroll/runs`, routes/payroll-runs.js + lib/payroll.js)
- **Import = Traffio "Person Dockets" CSV.** The export ranges by booking END, so it carries the previous week's Sunday-night shifts. `inferPeriod` picks the Mon–Sun week holding the MOST shifts (not the earliest date) so the run is labelled right. **Every shift in the file is paid once, on the weekday it STARTED** — a Sunday 19:00–03:00 shift is one Sunday shift (Saadat: "any shift starting Sunday, doesn't matter if they finish 8 days later, it's one shift"). Shifts dated outside the week are included by default and listed on the run (`import_summary_json.included_outside_period`) for a double-pay check; the import form's `skip_outside_period` tick drops them instead (`skipped_outside_period`). Never split a shift across days.
- **OT/DT bands are cumulative per calendar day** (`buildBuckets` → `splitShiftCumulative`): a second docket on the same date continues from the first. Day/night is by clock time (06–18 day). TFN ≤8 normal / 8–10 OT / >10 DT; ABN ≤8 / OT; Cash flat.
- **Missing OT/DT rates are never hidden**: hours stay in the OT/DT row, the rate borrows the tier below (`applyTierRateFallbacks`) and `buckets_json._warnings` flags it (rendered under the worker name + a run banner). `_warnings` is a non-bucket key — every loop iterates `BUCKETS`, so leave it be. TFN Night OT / Night DT are real rates (Worker Rates grid + tier presets stamp base × 1.5 / × 2; mig 355 backfilled them) — Saadat: "night OT is NECESSARY so is DT".
- **`travel` bucket** = Traffio `travel_time` hours at `employees.rate_travel` (falls back to the Day rate), outside the OT bands. Different from the `travel_allowance` (fares $/day, TFN).
- **Allowance modes** on `pay_run_lines`: `travel_override`/`meal_override` = 1 means manual (custom amount, or removed = $0). Line save, `/match` and `/refresh` only recompute rate × count when the flag is 0. The edit modal posts `travel_mode`/`meal_mode` = auto|custom|off.
- **Linking sticks**: `POST /lines/:id/match` stamps `employees.traffio_person_id` (mig 354) and `matchEmployee` checks it first, before `employee_reference` and name matching. Duplicate names ("Subcontractor 1") only ever need linking once.
- The run page saves the edit modal over fetch and restores the `<main>` scroll position on reload (the layout scrolls `main`, not `window`); redirects carry `#line-<id>`.

## Fleet fuel cards (routes/fleet.js "FUEL CARD" block, Sep 2026)
Each vehicle can hold one fuel card (mig 362: `vehicles.fuel_card_provider / fuel_card_last4 / fuel_card_number_enc / fuel_card_pin_enc / fuel_card_updated_at / _by`). Number and PIN are AES-256-GCM encrypted via `services/encryption.js` (same key as TFN/bank — `FIELD_ENCRYPTION_KEY`); only the last four digits are in the clear. The vehicle detail overview renders a masked "Fuel Card" block; `GET /fleet/:id/fuel-card/reveal` returns the plaintext JSON on demand and writes an `activity_log` row (`action: 'download'`) every time; the page re-masks after 30 s. `POST /fleet/:id/fuel-card` saves (blank number/PIN on an edit keep the current values; `action=clear` removes the card). Never log or render the PIN.

## Fleet tolls (`/fleet/tolls`, routes/fleet.js "TOLL INVOICES" block)
- The quarterly NSW E-Toll Statement/Tax Invoice PDF is uploaded on the hub, parsed by `services/tollInvoiceParser.js` (pdfjs text positions; DB-free so the tenant lint ratchet stays put) into per-tag / per-plate sections of trips, and stored in `toll_invoices` (mig 357: front-page figures + `summary_json` for lists + full `parsed_json` for the review). The PDF lives in **`data/toll-invoices/`** — persistent volume, but deliberately NOT under the public `/data/uploads` static mount; it's streamed only by the authed `GET /fleet/tolls/:id/file`.
- Review modal (`views/fleet/_toll_review.ejs`, opened by `?review=<id>`) matches sections to vehicles via `lib/tollMatch.js` (tag → `vehicles.toll_tag`, plate → `vehicles.rego`, normalised; Active beats Verify; still-ambiguous → user picks). "Add to vehicles" posts `include[key]`/`assign[key]`/`keep[key][row]`/`amt[key][row]`; the server rewrites every section from the form (delete + insert into `toll_trips`), so un-ticking removes trips and re-assigning moves them. A tag section assigned to a vehicle with a blank `toll_tag` sets it. Applied state is DERIVED from `toll_trips` — never stored.
- Several PDFs can be dropped at once (`tollUpload.array`, 12 max): each is parsed and filed by its statement period; one file → straight to its review, several → the list with a "Read N statements, filed by period" summary (duplicates by invoice number are skipped, a lone duplicate opens the existing one). Period detection: long-form range, then numeric range, then derived from the earliest/latest trip (`periodSource`). Lists and the review lead with the period, not the invoice number; the review footer links to the next statement still needing attention.
- E-Toll's own summary and detailed listing disagree on genuine statements (video-matched trips move between plate and tag); the listed trips are what gets attributed and the difference is a soft note, not an error. The rotated document id in the right margin (`664ROPO_…`, x>575) shares a baseline with rows and must be dropped before reading amounts.
- **Office staff cars** (`toll_ref_marks`, mig 358; `POST /fleet/tolls/marks` add/remove, JSON-aware): a tag/plate marked as a personal car is never flagged "Not in register", counts toward a statement being done, and is listed in a collapsed panel on the hub — but the vehicle select and "+ Add vehicle" stay on its row, and a real match/allocation always wins over the mark.
...
- **Overlays must live under `<body>`.** `#rec-root` ends up carrying a transform after the page-enter motion, and a transformed ancestor is the containing block for `position: fixed` — so a fixed modal/toast rendered inside the page centres on the page box, not the viewport. On a phone, once `<main>` had been scrolled, the "Book induction" date modal opened ~600px ABOVE the screen and a stage move looked like it silently failed. `_recruitment-script.ejs` re-parents `#booking-modal`/`#closed-modal`/`#backward-modal`/`#rec-toast` to `<body>` at init (the tap-to-move sheet always did). Any new fixed overlay on this page must do the same; `.rec-modal .rec-input` keeps the input styling outside `#rec-root`.
- **Public induction form** (`views/induction/form.ejs`, `POST /induction(/:type)/submit`): the email is typed twice (`email` + `confirm_email`, paste blocked on the second). Step 2 won't advance on a mismatch/malformed address, and `handleSubmission` rejects it server-side BEFORE the signature/consent and DOB checks — the form has `novalidate` and posts over fetch, so the server check is the one that counts. Stored email is the trimmed first box.
