...
- **Import = Traffio "Person Dockets" CSV.** The export ranges by booking END, so it carries the previous week's Sunday-night shifts. `inferPeriod` picks the Mon–Sun week holding the MOST shifts (not the earliest date) and `partitionShiftsByPeriod` drops shifts dated outside it — they're listed on the run (`pay_runs.import_summary_json`) and never bucketed. Don't "fix" this by paying them.
