...
- **Missing OT/DT rates are never hidden**: hours stay in the OT/DT row, the rate borrows the tier below (`applyTierRateFallbacks`) and `buckets_json._warnings` flags it (rendered under the worker name + a run banner). `_warnings` is a non-bucket key — every loop iterates `BUCKETS`, so leave it be.
